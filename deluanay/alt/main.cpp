...
int InCircle(triangle *t, int d)
{
    ...
    return (xcoor[a] * xcoor[a] + ycoor[a] * ycoor[a]) * TriArea(b, c, d) -
               ...
               (xcoor[d] * xcoor[d] + ycoor[d] * ycoor[d]) * TriArea(a, b, c) >
           0;
}
...
{
    if (!ccw(a1, b1, c1))
    { // check if the three points are in ccw sense
        int temp;
        temp = b1;
        b1 = c1;
        c1 = temp;
...
int legalizeedge(int pr, Edge *pipj, triangle *t)
{
    ...
    if (InCircle(pipj->leftface, v4))
    {
        ...
        e1->leftface = f1;
        if (e1->dualedge != NULL)
            e1->dualedge->rightface = f1;
        e2->leftface = f2;
        if (e2->dualedge != NULL)
            e2->dualedge->rightface = f2;
        e3->leftface = f2;
        if (e3->dualedge != NULL)
            e3->dualedge->rightface = f2;
        e4->leftface = f1;
        if (e4->dualedge != NULL)
            e4->dualedge->rightface = f1;
